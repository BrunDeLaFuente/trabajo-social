...
export default function ArticulosEditar() {
  ...
  // Cargar datos del artículo y títulos existentes
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);

        // Cargar todos los artículos para validar títulos únicos
        const articulosResponse = await api.get("/articulos");
        const allTitles = articulosResponse.data.map((articulo) =>
          articulo.titulo_noticia.toLowerCase()
        );

        // Encontrar el artículo actual
        const currentArticle = articulosResponse.data.find(
          (art) => art.id_noticia === Number.parseInt(id)
        );

        if (!currentArticle) {
          agregarMensaje("fail", "#D32F2F", "Artículo no encontrado");
          navigate(-1);
          return;
        }
        ...
        const filteredTitles = allTitles.filter(
          (title) => title !== currentArticle.titulo_noticia.toLowerCase()
        );
        setExistingTitles(filteredTitles);

        // Establecer datos del formulario
        setFormData({
          titulo_noticia: currentArticle.titulo_noticia,
          contenido: currentArticle.contenido,
          autor: currentArticle.autor || "",
          es_publico: Boolean(currentArticle.es_publico),
        });

        // Establecer archivos existentes
        setExistingImagenes(currentArticle.imagenes || []);
        setExistingVideos(currentArticle.videos || []);
        setExistingArchivos(currentArticle.archivos || []);

        // Guardar datos originales para comparación
        setOriginalData(currentArticle);

        // Establecer contenido en el editor
        setTimeout(() => {
          if (editorRef.current) {
            editorRef.current.innerHTML = currentArticle.contenido;
          }
        }, 100);
      } catch (error) {
        console.error("Error al cargar datos:", error);
        agregarMensaje("fail", "#D32F2F", "Error al cargar el artículo");
        navigate(-1);
      } finally {
        setLoading(false);
      }
  ...
  const validateForm = () => {
    ...
    if (!formData.titulo_noticia.trim()) {
      ...
      newErrors.titulo_noticia = "Ya existe un artículo con este título";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post(`/articulosActualizar/${id}`, formDataToSend, {
        headers: {
      ...
      agregarMensaje("fail", "#D32F2F", "Error al actualizar el artículo");
    } finally {
      setSaving(false);
    }
  };
    ...
    <Container>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Section>
          ...
          <FormGroup>
            <Label htmlFor="titulo_noticia">Título del artículo *</Label>
            <Input
              type="text"
              id="titulo_noticia"
              name="titulo_noticia"
