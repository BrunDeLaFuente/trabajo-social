...
export default function ArticulosCrear() {
  ...
  useEffect(() => {
    const fetchExistingTitles = async () => {
      try {
        ...
        setExistingTitles(
          response.data.map((articulo) => articulo.titulo_noticia.toLowerCase())
        );
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post("/articulosCrear", formDataToSend, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      agregarMensaje("success", "#2E7D32", "Artículo creado correctamente");
      ...
      agregarMensaje("fail", "#D32F2F", "Error al crear el artículo");
    } finally {
      setSaving(false);
    }
  };
