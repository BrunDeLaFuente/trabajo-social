...
export default function ComunicadosEditar() {
  ...
  // Cargar datos del comunicado y títulos existentes
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);

        // Cargar todos los comunicados para validar títulos únicos
        const comunicadosResponse = await api.get("/comunicados");
        const allTitles = comunicadosResponse.data.map((comunicado) =>
          comunicado.titulo_noticia.toLowerCase()
        );

        // Encontrar el comunicado actual
        const currentComunicado = comunicadosResponse.data.find(
          (com) => com.id_noticia === Number.parseInt(id)
        );

        if (!currentComunicado) {
          agregarMensaje("fail", "#D32F2F", "Comunicado no encontrado");
          navigate(-1);
          return;
        }
        ...
        const filteredTitles = allTitles.filter(
          (title) => title !== currentComunicado.titulo_noticia.toLowerCase()
        );
        setExistingTitles(filteredTitles);

        // Establecer datos del formulario
        setFormData({
          titulo_noticia: currentComunicado.titulo_noticia,
          contenido: currentComunicado.contenido,
          autor: currentComunicado.autor || "",
          es_publico: Boolean(currentComunicado.es_publico),
        });

        // Establecer archivos existentes
        setExistingImagenes(currentComunicado.imagenes || []);
        setExistingVideos(currentComunicado.videos || []);
        setExistingArchivos(currentComunicado.archivos || []);

        // Guardar datos originales para comparación
        setOriginalData(currentComunicado);

        // Establecer contenido en el editor
        setTimeout(() => {
          if (editorRef.current) {
            editorRef.current.innerHTML = currentComunicado.contenido;
          }
        }, 100);
      } catch (error) {
        console.error("Error al cargar datos:", error);
        agregarMensaje("fail", "#D32F2F", "Error al cargar el comunicado");
        navigate(-1);
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post(`/comunicadosActualizar/${id}`, formDataToSend, {
        headers: {
      ...
      agregarMensaje("fail", "#D32F2F", "Error al actualizar el comunicado");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <LoadingSpinner message="Cargando comunicado..." />;
  }
